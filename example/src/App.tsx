...
export default function App() {
  ...
  const [showStrokeWidthPopover, setStrokeWidthShowPopover] = useState(false);
  const [showColorPopover, setColorShowPopover] = useState(false);
  const [visiblePaintMode, setVisiblePaintMode] = useState(false);
  const [showMimeTypePopover, setShowMimeTypePopover] = useState(false);
  const [paintMode, setPaintMode] = useState(PaintMode.PEN);
  const [selectedColor, setSelectedColor] = useState(generateColors[0]);
  const [mimeType, setMimeType] = useState(MimeType.png);
    ...
    <PaperProvider>
      <View style={styles.container}>
        ...
        >
          ...
          />
          <PaperDropdown
            visible={visiblePaintMode}
            setVisible={setVisiblePaintMode}
            value={paintMode}
            ...
            anchor={
              <Icon
                name={paintModes.get(paintMode) ?? 'pen'}
                size={30}
                onPress={() => setVisiblePaintMode(true)}
              />
            }
          />
          <Popover
            isVisible={showColorPopover}
            backgroundStyle={{
            ...
            onRequestClose={() => setColorShowPopover(false)}
            from={
              <View>
                <Icon
                  name="circle"
              ...
              {generateColors.map((c, i) => (
                <Pressable key={i} onPress={async () => {
                  ...
                  <Icon
                    ...
                    color={c}
                    style={{
                      borderWidth: selectedColor === c ? 0.4 : 0,
                      borderColor: '#000',
                      borderRadius: 20
                    }}
                  />
          ...
          <Popover
            isVisible={showStrokeWidthPopover}
            backgroundStyle={{
              backgroundColor: 'transparent',
            }}
            placement={Placement.BOTTOM}
            ...
            onRequestClose={() => setStrokeWidthShowPopover(false)}
            from={
              <View>
                <Icon
          ...
          >
            <View style={{ width: 140, justifyContent: 'center', flexDirection: 'row' }}>
              <IconButton icon={'minus'} onPress={() => {
                const newSize = strokeWidth - 1;
                setStrokeWidth(newSize);
                paintEditor1.setStrokeSize(newSize);
              }} />
              <Text style={{ alignSelf: 'center', marginHorizontal: 8 }}>{strokeWidth}</Text>
          ...
          <Popover
            ...
            onRequestClose={() => setShowMimeTypePopover(false)}
            from={
              <View>
                <Icon
                  name={'folder-outline'}
                  size={30}
                  onPress={() => setShowMimeTypePopover(true)}
                />
              </View>
            }
          >
            <View style={{ width: 200, height: 240, alignItems: 'center', justifyContent: 'center' }}>
              {
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' }}>
                  {Object.values(MimeType).map((it) =>
                    <Button mode={it === mimeType ? 'contained-tonal' : 'text'} style={{ borderRadius: 8, marginHorizontal: 8, marginTop: 4, width: 80 }} onPress={() => {
                      setMimeType(it);
                    }} >{it}
                    </Button>
                  )}
              ...
              <Text>export.{mimeType}</Text>
              <Button mode={'contained-tonal'} style={{ borderRadius: 8, margin: 8 }} onPress={() => {
                try {
                  paintEditor1.export(DocumentDirectoryPath, 'export', mimeType);
                  setSnackbar({
                    visible: true,
                    message: `File saved in ${DocumentDirectoryPath}/export.${mimeType}`,
                  });
                } catch (err) {
                  console.error(err);
                  setSnackbar({
